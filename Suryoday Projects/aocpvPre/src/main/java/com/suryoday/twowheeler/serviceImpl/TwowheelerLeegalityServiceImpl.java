...
//		String base64pdf=jsonObject.getJSONObject("Data").getString("Base64Pdf");
	...
	private JSONObject getRequest(String applicationNo, String base64pdf, String documentType) {
		...
		if(documentType.equals("loanAgreement")) {
			data.put("ProfileId", "fsngBCC");
			data.put("StampValue", "20");
		}else if(documentType.equals("sanctionLetter")) {
			data.put("ProfileId", "cVtVN1m");
		}
		User user = userService.fetchById(byApplication.getSalesCreatedBy());
		
		JSONObject file = new JSONObject();
		file.put("Name", user.getUserName());
		...
		cc1.put("Name", user.getUserName());
		cc1.put("Email",user.getEmailId());
	
		cc.put(cc1);
		data.put("CC", cc);
		data.put("Irn", byApplication.getApplicationNo());
	...
	JSONArray invitees = data.getJSONObject("Data").getJSONArray("Invitees");
		...
		for(int n=0;n<invitees.length();n++) {
			...
			if(!jsonObject.get("SignUrl").equals(null)) {
				...
				LeegalityInfo info =new LeegalityInfo(applicationNo,Active,Email,ExpiryDate,Name,Phone,SignUrl,documentId,"false",documentType);
				list.add(info);
			}
		}
		leegalityInfoRepository.saveAll(list);
			...
			for(LeegalityInfo leegalityInfo:listDB) {
				leegalityInfo.setDocumentId(documentId);
				leegalityInfoRepository.save(leegalityInfo);
			}
		}
	}
