...
public class DsaOnboardIfscCodeEncy {
	...
	public ResponseEntity<Object> pennyDrop(@RequestBody String bm,
		...
		Header.put("X-Request-ID", X_Request_ID);
			...
			if(dsaOnboardDetails.getBankDetailsVerify()!=null && dsaOnboardDetails.getBankDetailsVerify().equals("YES")) {
				throw new NoSuchElementException("Bank Details already verified");
			}
			...
			if (pennyDrop != null) {
				...
				dsaOnboardDetails.setBankDetailsResponse(Data1.toString());
				dsaOnboardDetails.setBankDetailsVerify(status);
