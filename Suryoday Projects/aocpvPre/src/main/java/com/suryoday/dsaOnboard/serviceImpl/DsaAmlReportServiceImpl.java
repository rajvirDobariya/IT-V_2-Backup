...
public class DsaAmlReportServiceImpl implements DsaAmlReportService{
	...
	public JSONObject getAmlReport(String applicationNo, String member, JSONObject header) {
		...
		try {
			...
			obj = new URL(x.BASEURL+"aml/query/v2?api_key="+x.api_key);
			logger.debug(x.BASEURL+"aml/query/v2?api_key="+x.api_key);
	...
	private JSONObject getRequest(String applicationNo, String member) {
		...
		if(dsaOnboardMember.getDateOfBirth()!= null) {
			DateTimeFormatter inputFormatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
	        DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	        
	        LocalDate date = LocalDate.parse(dsaOnboardMember.getDateOfBirth(), inputFormatter);
	        String dob = date.format(outputFormatter);
	        customer.put("DateOfBirth", dob);
		}
