...
public class DsaImageServiceImpl implements DsaImageService{
	...
	public void saveImage(MultipartFile[] files, String applicationNo, JSONArray document, String member) {
		...
		for(MultipartFile file : files ) {
			...
			Optional<DsaImage> optional=imageRepository.getByApplicationNoAndDocumentType(applicationNo, Documenttype);
			...
			for (int n = 0; n < document.length(); n++) {
				...
				if (fileName.equals(fileNameInRequest)) {
					try {
						...
						Optional<DsaImage> optional = imageRepository.getByApplicationNoAndDocumentTypeAndMember(applicationNo, Documenttype,member);
						if (optional.isPresent()) {
