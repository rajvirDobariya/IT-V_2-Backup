...
public class LeegalityServiceImpl implements LeegalityService{
	...
	public JSONObject leegalityCheck(long applicationNoInLong, JSONObject header) {
		...
		try {
			...
			if(Data1.has("Data")) {
				...
				loanCreation.setSanctionLetterIsVerify("NO");
				loanCreation.setAggreementLetterIsVerify("NO");
	...
	public JSONObject statusCheck(long applicationNoInLong, JSONObject header) {
		...
		try {
			...
			org.json.JSONObject request = new org.json.JSONObject();
			HttpsURLConnection.setDefaultHostnameVerifier(allHostsValid);
			logger.debug(x.BASEURL + "leegality/status/check/"+loanCreation.getDocumentId()+"?api_key=" + x.api_key);
			obj = new URL(x.BASEURL + "leegality/status/check/"+loanCreation.getDocumentId()+"?api_key=" + x.api_key);
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
			...
			if(Data1.has("Data")) {
				...
				if(count==0) {
					...
					loanCreation.setDocumentVerify("Online");
					aocpvLoanCreationService.saveData(loanCreation);
