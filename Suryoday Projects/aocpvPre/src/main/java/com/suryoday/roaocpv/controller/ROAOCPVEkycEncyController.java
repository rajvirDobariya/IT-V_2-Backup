		...
		if (sessionId == true) {
			...
			if (jsonAPIResponse.has("Response")) {
				...
				String decryptString = roaocpvEkycService.decryptString(hsmData);
					...
					org.json.JSONObject DBResponse = new org.json.JSONObject();
					if (!jsonResponse.equals(null)) {
						...
						//	JSONObject poi = uidData.getJSONObject("Poi");
							...
							DBResponse.put("Response", responseObj);
							DBResponse.getJSONObject("Response").getJSONObject("KycRes").getJSONObject("UidData").remove("Pht");
							applicationDetailsService.saveData("ekyc", uid, applicationNo, DBResponse.toString());
		...
		if (sessionId == true) {
			...
			if (jsonAPIResponse.has("Response")) {
				...
				String decryptString = roaocpvEkycService.decryptString(hsmData);
					...
					String errorResponse=errorResponseService.getError(kycRes,Long.parseLong(applicationNo));
					data = errorResponse;
					//logger.debug("Bio matric error"+data);
					String encryptString2 = AppzillonAESUtils.encryptStringtoContainer(data, key);
					org.json.JSONObject data2 = new org.json.JSONObject();
					data2.put("value", encryptString2);
					org.json.JSONObject data3 = new org.json.JSONObject();
...
//					uidData.remove("Pht");
				...
				applicationDetailsService.saveData("ekyc", aadharNo, applicationNo, DBResponse.toString());
