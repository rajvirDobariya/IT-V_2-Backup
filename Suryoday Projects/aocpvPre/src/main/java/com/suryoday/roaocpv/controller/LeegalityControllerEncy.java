...
public class LeegalityControllerEncy {
	...
	@RequestMapping(value = "/leegalityEncy", method = RequestMethod.POST, produces = "application/json")
	public ResponseEntity<Object> faceMatch(@RequestBody String jsonRequest,
		...
		JSONObject leegality = leegalityService.leegalityCheck(applicationNoInLong ,Header);

		HttpStatus h = HttpStatus.BAD_GATEWAY;
		if (leegality != null) {
			String Data2 = leegality.getString("data");
			...
			if (Data1.has("Data")) {
				...
				response.put("Irn", Data1.getJSONObject("Data").getString("Irn"));
				response.put("TransactionCode", Data1.getJSONObject("Data").getString("TransactionCode"));
				response.put("DocumentId", Data1.getJSONObject("Data").getString("DocumentId"));
				response.put("is_verify", "NO");
	...
	public ResponseEntity<Object> statusCheck(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
			@RequestHeader(name = "X-Correlation-ID", required = true) String X_Correlation_ID,
		...
		JSONObject leegality = leegalityService.statusCheck(applicationNoInLong ,Header);
		...
		if (leegality != null) {
			...
			if (Data1.has("Data")) {
				...
				response.put("TransactionCode", Data1.getJSONObject("Data").getString("TransactionCode"));
				response.put("DocumentId", Data1.getJSONObject("Data").getString("DocumentId"));
				response.put("is_verify", leegality.getString("is_verify"));
				String data = response.toString();
				String encryptString2 = AppzillonAESUtils.encryptStringtoContainer(data, key);
				org.json.simple.JSONObject data2 = new org.json.simple.JSONObject();
				data2.put("value", encryptString2);
