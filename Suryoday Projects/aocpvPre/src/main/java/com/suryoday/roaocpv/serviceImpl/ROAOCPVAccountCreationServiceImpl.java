...
public class ROAOCPVAccountCreationServiceImpl implements ROAOCPVAccountCreationService {
	...
	public JSONObject accountCreation(JSONObject jsonObject, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURL + "account/CASA?api_key=" + x.api_key);
		//	obj= new URL("https://intramashery.suryodaybank.co.in/ssfbuat/account/CASA/v2?api_key=kyqak5muymxcrjhc5q57vz9v");
			obj = new URL(x.BASEURL + "account/CASA?api_key=" + x.api_key);
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
			con.setRequestMethod("POST");
			con.setRequestProperty("Content-Type", "application/json");
			con.setRequestProperty("X-Request-ID", "TB1");
			con.setRequestProperty("X-User-ID", header.getString("X-User-ID"));
			con.setRequestProperty("X-From-ID", "CB");
			con.setRequestProperty("X-To-ID", "IEX");
	...
	private JSONObject getRequest(Long applicationNo1) {
		...
		if (byApplicationNo.isPresent()) {
			...
			AocpvIncomeDetails aocpvIncomeDetails = find.get();
			 ...
			 AocpvLoanCreation aocpvLoanCreation = aocpvLoanCreationService.findByApplicationNo(applicationNo1);
			...
			customer.put("NamePrefix","1"); // aocpCustomer.getTitle()
...
//			
			...
			String debitcard="";
			String InternateBanking="";
			String MobileBanking="";
			String EStatement="";
			String SmsAlerts="";
			String ChequeBook="";
			String SweepFacility="";
			...
			customer.put("MobileNumber", aocpCustomer.getMobileNo());
