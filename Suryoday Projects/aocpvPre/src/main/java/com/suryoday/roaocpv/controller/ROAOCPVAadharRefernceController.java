...
public class ROAOCPVAadharRefernceController {

	@Autowired
	ROAPCVAadharReferenceService aadharReferenceService;

	@Autowired
	ApplicationDetailsService applicationDetailsService;

	private static Logger logger = LoggerFactory.getLogger(ROAOCPVAadharRefernceController.class);
			...
			if (createAadharReference != null) {
				...
				if (Data1.has("Data")) {
					h = HttpStatus.OK;
					 ...
					 applicationDetailsService.validateAadharCard(referenceNo);
//					applicationDetailsService.saveData("aadharCard", referenceNo, applicationNo, Data2);
