...
public class WhatsappMediaSendServiceImpl implements WhatsappMediaSendService {
	...
	public JSONObject sendMedia(JSONObject jsonObject, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURL + "whatsapp/media/template/push?api_key=" + x.api_key_whatsapp);
			obj = new URL(x.BASEURL + "whatsapp/media/template/push?api_key=" + x.api_key_whatsapp);
	...
	private JSONObject getRequest(String applicationNo,String documentType) {
		...
		MediaTemplate.put("Filename",documentType+"_"+aocpCustomer.getName()+"_"+aocpCustomer.getApplicationNo());
	...
	public JSONObject sendMessage(JSONObject jsonObject, JSONObject header) {
		...
		try {
			...
			HttpsURLConnection.setDefaultHostnameVerifier(allHostsValid);
			logger.debug(x.BASEURL + "whatsapp/media/template/push?api_key=" + x.api_key);
			obj = new URL(x.BASEURL + "whatsapp/media/template/push?api_key=" + x.api_key);
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
			con.setRequestMethod("POST");
			con.setRequestProperty("Content-Type", "application/json");
			con.setRequestProperty("X-Correlation-ID", header.getString("X-Correlation-ID"));
	...
	private JSONObject getRequestforMessage(String applicationNo) {
		...
		Parameter2.put("text", "https://https://sarathi.suryodaybank.com/");
		body.add(Parameter1);
		body.add(Parameter2);
