...
public class PDFAo {
	...
	//private List<AocpvLoanCreation> appln3;
	...
	public byte[] exportPDFAO(HttpServletResponse res) throws  Exception {
		...
		PdfWriter writer = PdfWriter.getInstance(document, stream);
         ...
         Image image = Image.getInstance("//opt//tomcat-suryodaysarathi//apache-tomcat-9.0.37//conf//logo.jpg");
...
//         document.add( Chunk.NEWLINE );
	     ...
	     for(AocpCustomer application : appln) {
	    	 ...
	    	 String nomineeFrnd = nominee.getString("relationship");
...
//	     {
	     ...
	     String title = " Application Reference Number                    "+list1.get(0)+"                                        Branch ID           "+list1.get(8)+"";                            
	     ...
	     String line15 = " Nominee Name                  "+list1.get(3)+"                                             Relationship with Depositor            "+list1.get(5)+"";                                
	     String line16 = " Nominee Date of Birth       "+list1.get(4);                                   
	     String line17 = " Nominee Address              SAME AS APPLICANTS PERMANENT ADDRESS";                               
	     String line18 = " "+day+", "+now+"                                         AO_Form                                                          1";                                   
	      
	     document.add(new Paragraph(title,font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(line1,font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph("____________________________________________________________________________________________",font2));
	     document.add(new Paragraph(" Declaration",font2));
	     document.add(new Paragraph("____________________________________________________________________________________________",font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(" I/We agree to open Savings Ujjwal (1003) account with the balance requirement of Rs.0.00.I/We fully",font2));
	     document.add(new Paragraph(" understand the detailed charges (including the balance non-maintenance charges) \"applicable to this",font2));
	     document.add(new Paragraph(" account as per, Schedule of Charges which is available on the Bank's Website / Branch notice board.",font2));
	     ...
	     document.add(new Paragraph("____________________________________________________________________________________________",font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(" I/We NALINI NALINI, residing at W/O RAJIVGANTHI,465 MARIYAMAN KOVIL ST, TIRUVAMUR,",font2));
	     document.add(new Paragraph(" PERIYAKALANI, PANRU, CUDDALORE, TAMIL NADU, INDIA - 607106 Nominate the following",font2));
	     document.add(new Paragraph(" person to whom in the event of my/our/minor's death the amount of deposit, may be returned by",font2));
	     ...
	     document.add(new Paragraph("____________________________________________________________________________________________",font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(" • All necessary documentation as mandated by the Regulatory/Bank authorities should be provided for",font2));
	     document.add(new Paragraph("   opening the accounts.",font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(" • All accounts should maintain the stipulated average balance based on the product/program and branch in",font2));
	     document.add(new Paragraph("   which the account is opened.",font2));
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(" • In case of non-maintenance of the stipulated average balance, charges as outlined in the Schedule of",font2));
	     ...
	     document.add( Chunk.NEWLINE );
	     document.add(new Paragraph(" • In case of any query / suggestion / feedback / complaint relating to features of any of the products, you may",font2));
