...
public class PanValidationControllerEncy {
	private static Logger logger = LoggerFactory.getLogger(PanValidationControllerEncy.class);
	...
	public ResponseEntity<Object> panCardValidation(
		...
		if (sessionId == true) {
			...
			if (X_Request_ID.equals("IEXCEED")) {
				...
				JSONObject panCardValidate = panValidationService.panCardValidation(panCardNo, Header);
				...
				if (panCardValidate != null) {
					...
					if (Data1.has("Data")) {
						...
						h = HttpStatus.OK;
						customerDataService.validatePancard(panCardNo);
					}
