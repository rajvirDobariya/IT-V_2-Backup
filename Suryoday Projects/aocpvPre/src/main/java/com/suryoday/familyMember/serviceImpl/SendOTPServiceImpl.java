...
public class SendOTPServiceImpl implements SendOTPService{
	...
	public JSONObject sendOtp(JSONObject jsonObject, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURLV2+"notification/otp/sms?api_key="+x.api_key);
	
			 obj =new URL(x.BASEURLV2+"notification/otp/sms?api_key="+x.api_key);
		...
		try {
			...
			// Create all-trusting host name verifier
			 HostnameVerifier allHostsValid = new HostnameVerifier() {
				...
				//String url="https://intramashery.suryodaybank.co.in/transaction/OTP/validate?api_key=kyqak5muymxcrjhc5q57vz9v&OTP=";
							int otp=Integer.parseInt(oTP);
					//				String transactionType="&TransactionType=D";
				logger.debug(x.BASEURLV2+"transaction/OTP/validate?api_key="+x.api_key+"&OTP="+oTP+"&TransactionType=D");
				obj =new URL(x.BASEURLV2+"transaction/OTP/validate?api_key="+x.api_key+"&OTP="+oTP+"&TransactionType=D");
					HttpURLConnection con = (HttpURLConnection) obj.openConnection();
