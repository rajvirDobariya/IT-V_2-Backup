...
import com.suryoday.aocpv.pojo.BenificiaryInfo;
import com.suryoday.aocpv.pojo.Benificier;
import com.suryoday.aocpv.service.BenificierService;
...
public class BenificierController {
  ...
  public List<Benificier> getalldata() throws ResourceNotFoundException {
    ...
    return this.benificierService.fetchAll();
  }
  
  @RequestMapping(value = {"/customer/{customerid}/beneficiary/view/{accounttype}"}, method = {RequestMethod.GET}, produces = {"application/json"})
  public ResponseEntity<Object> getalldataByCif(@PathVariable(name = "customerid") String customerid, @PathVariable(name = "accounttype") String accounttype, @RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist, @RequestHeader(name = "X-From-ID", required = true) String X_From_ID, @RequestHeader(name = "X-To-ID", required = true) String X_To_ID, @RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID, @RequestHeader(name = "X-User-ID", required = true) String X_User_ID, @RequestHeader(name = "X-Request-ID", required = true) String X_Request_ID) throws Exception {
    System.out.println("welcome");
    ...
    List<Benificier> l = new ArrayList<>();
    List<BenificiaryInfo> BenificiaryInfo1 = new ArrayList<>();
    l = this.benificierService.viewcustmerbytype(customerid, acc_type);
    if (l.size() == 0) {
    ...
    for (int i = 0; i < l.size(); i++) {
      ...
      b1.setSequence(Integer.valueOf(Integer.parseInt(b.getSequence())));
  ...
  public ResponseEntity<Object> insertdata(@RequestBody String bm, @PathVariable(name = "customerid") String customerid, @RequestHeader(value = "User-Agent", required = true) String userAgent, @RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist, @RequestHeader(name = "X-From-ID", required = true) String X_From_ID, @RequestHeader(name = "X-To-ID", required = true) String X_To_ID, @RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID, @RequestHeader(name = "X-User-ID", required = true) String X_User_ID, @RequestHeader(name = "X-Request-ID", required = true) String X_Request_ID) throws Exception {
    if (X_From_ID.equals("IBR") || X_From_ID.equals("MB")) {
      ...
      JSONObject s = this.benificierService.saveBenificier(bm.toString(), customerid);
      return new ResponseEntity(s.toString(), HttpStatus.OK);
    } 
    return new ResponseEntity("Invalid X_From_ID", HttpStatus.BAD_REQUEST);
  }
  
  @RequestMapping(value = {"/customer/{customerid}/beneficiary/modify"}, method = {RequestMethod.PUT}, produces = {"application/json"})
  public ResponseEntity<Object> modifydata(@RequestBody String bm, @PathVariable(name = "customerid") String customerid, @RequestHeader(value = "User-Agent", required = true) String userAgent, @RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist, @RequestHeader(name = "X-From-ID", required = true) String X_From_ID, @RequestHeader(name = "X-To-ID", required = true) String X_To_ID, @RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID, @RequestHeader(name = "X-User-ID", required = true) String X_User_ID, @RequestHeader(name = "X-Request-ID", required = true) String X_Request_ID) throws Exception {
    if (X_From_ID.equals("IBR") || X_From_ID.equals("MB")) {
      System.out.println("User agent is: {}" + userAgent);
      System.out.println("modify" + bm.toString());
      System.out.println("api key " + customerid);
      JSONObject s = this.benificierService.modifyBenificier(bm.toString(), customerid);
      return new ResponseEntity(s.toString(), HttpStatus.OK);
    } 
    return new ResponseEntity("Invalid X_From_ID", HttpStatus.BAD_REQUEST);
  }
  ...
  public ResponseEntity<Object> deletebeneficiary(@RequestBody String bm, @RequestHeader(value = "User-Agent", required = true) String userAgent, @RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist, @RequestHeader(name = "X-From-ID", required = true) String X_From_ID, @RequestHeader(name = "X-To-ID", required = true) String X_To_ID, @RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID, @RequestHeader(name = "X-User-ID", required = true) String X_User_ID, @RequestHeader(name = "X-Request-ID", required = true) String X_Request_ID) throws Exception {
    if (X_From_ID.equals("IBR") || X_From_ID.equals("MB")) {
      System.out.println("delete" + bm.toString());
      JSONObject s = this.benificierService.deleteBenificier(bm.toString());
      return new ResponseEntity(s.toString(), HttpStatus.OK);
    } 
    return new ResponseEntity("Invalid X_From_ID", HttpStatus.BAD_REQUEST);
  }
  ...
  public void updatbenifierstatus() {
    ...
    this.benificierService.modifyBenificierStatus();
  }
}
