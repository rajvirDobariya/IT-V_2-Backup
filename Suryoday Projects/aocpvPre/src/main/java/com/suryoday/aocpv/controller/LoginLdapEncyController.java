...
//					throw new NoSuchElementException("You are Not authorized to View");
				...
				for (int i = 0; i < array.length(); i++) {
				 ...
				 List<String> access = dtos.stream()
                         .map(PermissionDto::getName)
                         .collect(Collectors.toList());
				 
				List<String> collect = mainResponseDto.getUserAccess().stream()
																	  .filter(e->access.contains(e))
																	  .collect(Collectors.toList());
				if(collect.contains("INSURANCE"))
				{
				collect.remove("INSURANCE");
				}
				if(!collect.contains("COLLECTION")) {
