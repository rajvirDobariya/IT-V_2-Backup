...
public interface LoanInputRepositery extends JpaRepository<LoanDetails, String>{
		...
		@Query(value = "SELECT new com.suryoday.aocpv.pojo.LoanDetail(l.mobilePhone, l.referenceNo, l.customerID, l.memberName, l.state, l.branchId, l.amount, l.status, l.createDate, l.productCode) from LoanDetails l where l.branchId =:b and l.productCode =:p and l.isActive='1' and l.status =:s order by l.updatedate desc")
		Optional<List<LoanDetail>> getByIdAndStatus(@Param("b") long branchId ,@Param("s") String status,@Param("p") String productCode);

		@Query(value = "SELECT a from LoanDetails a where a.customerID =:customerId and a.productCode =:product")
		public Optional<LoanDetails> getByCustomerID(long customerId,String product);
			
		@Query(value = "SELECT a from LoanDetails a where a.status =:s and a.productCode =:product")
		Optional<List<LoanDetails>> findByStatus(@Param ("s") String status,String product);
		
		@Query(value = "SELECT new com.suryoday.aocpv.pojo.LoanDetail(l.mobilePhone, l.referenceNo, l.customerID, l.memberName, l.state, l.branchId, l.amount, l.status, l.createDate, l.productCode) from LoanDetails l where l.customerID =:customerId and l.productCode =:productCode")
		Optional<LoanDetail> getByCustomerId(@Param("customerId") long customerId,String productCode);
		...
		@Query(value = "SELECT new com.suryoday.aocpv.pojo.LoanDetail(l.mobilePhone, l.referenceNo, l.customerID, l.memberName, l.state, l.branchId, l.amount, l.status, l.createDate, l.productCode) from LoanDetails l where l.customerID =:customerId and l.productCode =:productCode")
		List<LoanDetail> getByCustId(long customerId,String productCode);

		@Query(value = "SELECT a from LoanDetails a where a.customerID =:customerId and a.productCode =:product order by a.updatedate desc")
		List<LoanDetails> fetchByCustomerId(long customerId,String product);
