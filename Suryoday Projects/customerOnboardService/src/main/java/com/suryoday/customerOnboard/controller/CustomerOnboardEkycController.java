...
import com.suryoday.customerOnboard.others.AppzillonAESUtils;
import com.suryoday.customerOnboard.service.AadharReferenceService;
...
import com.suryoday.customerOnboard.service.EkycOtpService;
import com.suryoday.customerOnboard.service.EkycService;
import com.suryoday.customerOnboard.service.ErrorResponseService;
import com.suryoday.customerOnboard.service.ValidateEkycOtpService;

@RestController
public class CustomerOnboardEkycController {

	
	@Autowired
	EkycService roaocpvEkycService;

	@Autowired
	ErrorResponseService errorResponseService;
	
	@Autowired
	EkycOtpService otpservice;
	
	@Autowired
	ValidateEkycOtpService validateOtpService;
	...
	@Autowired
	CustomerOnboardImageService imageService;
	
	private static Logger logger = LoggerFactory.getLogger(CustomerOnboardEkycController.class);
	...
	public ResponseEntity<Object> ekycRequest(@RequestBody String parent,
		...
		boolean sessionId = onboardDetailsService.validateSessionId(X_Session_ID, req);
		if (sessionId == true) {
			onboardDetailsService.getSessionId(X_User_ID, req);
				...
				if (!hsmData.equals("")) {
					...
					if (!jsonResponse.equals(null)) {
						...
						if (jsonResponse.has("KycRes")) {
							...
							imageService.saveAadharPhoto(map, applicationNo, jsonArray);
							// uidData.remove("Pht");
							uidData.remove("Prn");
							...
							onboardDetailsService.saveResponse("ekyc", "FINGERPRINT", applicationNo,DBResponse.toString());
				...
				String decryptString = roaocpvEkycService.decryptString(hsmData);
					...
					if (!jsonResponse.equals(null)) {
						...
						if (jsonResponse.has("KycRes")) {
							...
							onboardDetailsService.saveResponse("ekyc", "fACEEKYC", applicationNo,DBResponse.toString());
...
//	 	String xmlRequest = otpservice.getXmlRequest(Long.toString(aadhar),stan);
		...
		if (!jsonResponse.equals(null)) {
			...
			if (jsonResponse.has("KycRes")) {	
				...
				onboardDetailsService.saveResponse("ekyc", "Otp", applicationNo,DBResponse.toString());
