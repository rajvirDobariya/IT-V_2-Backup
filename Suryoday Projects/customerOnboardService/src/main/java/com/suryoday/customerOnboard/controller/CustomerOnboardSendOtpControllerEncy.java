...
import com.suryoday.customerOnboard.service.SendOtpService;

@RestController
public class CustomerOnboardSendOtpControllerEncy {

	Logger logger = LoggerFactory.getLogger(CustomerOnboardSendOtpControllerEncy.class);
	...
	public ResponseEntity<Object> sendOtpAndEmail(@RequestBody String bm,
		...
		if (sessionId == true) {
			onboardDetailsService.getSessionId(X_User_ID, request);
		...
		String data = "";
			if (X_Request_ID.equals("NOVOPAY")) {
				...
				if (sendsms != null) {
					...
					if (Data1.has("Data")) {
						String applicationNo = onboardDetailsService.createLead(mobileNo,X_User_ID);
