...
public class ROAOCPVPanCardConsumingController {

	@Autowired
	PanValidationService panCardConsumingService;

	@Autowired
	ApplicationDetailsService applicationDetailsService;

	private static Logger logger = LoggerFactory.getLogger(ROAOCPVPanCardConsumingController.class);
	...
	public ResponseEntity<Object> panCardValidation(
		...
		if (X_Request_ID.equals("IEXCEED")) {
			...
			JSONObject panCardValidate = panCardConsumingService.panCardValidation(j, Header);
			...
			if (panCardValidate != null) {
				...
				if (Data1.has("Data")) {
					...
					h = HttpStatus.OK;
					// applicationDetailsService.validatePancard(panCardNo);
					applicationDetailsService.saveData("panCard", panCardNo, applicationNo, jsonArray1.toString());
