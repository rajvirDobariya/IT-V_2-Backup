	...
	public PersonalDetailResponse getPersonalDetailsByAppln(String appln) {
		...
		if (optional.isPresent()) {
			...
			if (personalDetailsReq != null) {
				...
				String CASTE = jsonObject.getString("Caste");
				String RELIGION = jsonObject.getString("Religion");
				PersonalDetailResponse response = new PersonalDetailResponse(title, FIRST_NAME, MIDDLE_NAME, LAST_NAME,
						MOBILE, DOB, AGE, GENDER, FATHER_NAME, MOTHER_NAME, SPOUSE_NAME, NO_OF_DEPENDENTS,
						QUALIFICATION, CASTE, RELIGION, marritialStatus);
				return response;
			}
		...
		if (optional.isPresent()) {
			...
			if (a.getAddressDetailsReq() != null) {
				...
				for (int n = 0; n < jsonArray.length(); n++) {
					...
					Address address = new Address(AddressType, address1, address2, pincode, city, district, state,
							landmark, address3);
					list.add(address);
				}
			}
	...
	public ApplicationDetailsResponse fetchAllByApplicationId(String applicationNo) {
		...
		if (optional.isPresent()) {
			...
			if (personalDetailsReq != null) {
				...
				String CASTE = jsonObject.getString("Caste");
				String RELIGION = jsonObject.getString("Religion");
				response1 = new PersonalDetailResponse(title, FIRST_NAME, MIDDLE_NAME, LAST_NAME, MOBILE, DOB, AGE,
						GENDER, FATHER_NAME, MOTHER_NAME, SPOUSE_NAME, NO_OF_DEPENDENTS, QUALIFICATION, CASTE, RELIGION,
						marritialStatus);
			}
			...
			List<AocpvImages> listAocpvImage = aocpvImageService.getByApplicationNoAnddocument(applicationNoInLong,
					"customerPhoto");
...
//		String applicationNo = LocalDateTime.now().toString().replace("-", "").replace("T", "").replace(":", "").substring(2,14);
		...
		Optional<String> optional = applicationDetailsRepository.fetchByapplicationId(applicationNo);
		if (optional.isPresent()) {
	...
	public List<ApplicationDetails> getByBreStatus(String status, String branchId, LocalDate startdate,
			LocalDate enddate) {
		List<ApplicationDetails> list = applicationDetailsRepository.getByBreStatus(status, branchId, startdate,
				enddate);
