...
@Component
@Service
public class ROAOCPVRepaymentServiceImpl implements ROAOCPVRepaymentService {
	private static Logger logger = LoggerFactory.getLogger(ROAOCPVRepaymentServiceImpl.class);

	@Override
	public JSONObject repayment(String AccountNumber, JSONObject header) {

		JSONObject request = getRequest(AccountNumber);
		...
		try {
			...
			logger.debug(x.BASEURL + "repayment/schedule/generation/v2?api_key=" + x.api_key);
			obj = new URL(x.BASEURL + "repayment/schedule/generation/v2?api_key=" + x.api_key);
	...
	private JSONObject getRequest(String AccountNumber) {
		...
		data.put("Frequency", "");
		data.put("Term", "");
		data.put("InterestRate", "");
		data.put("PrincipalMoratorium", "");
		data.put("InterestMoratorium", "");
		data.put("EmiChange", "");
		data.put("UserID", "");
