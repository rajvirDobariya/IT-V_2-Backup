...
public class ROAOCPVAccountCreationController {
	...
	Logger logger = LoggerFactory.getLogger(ROAOCPVAccountCreationController.class);
	...
	public ResponseEntity<Object> accountCreation(@RequestBody String bm,
		...
		JSONObject dedupeCall = roaocpvAccountCreationService.accountCreation(jsonObject, Header, findByApplicationNo);
		...
		logger.debug("pdf generation start" + string2);
		ResponseEntity<Object> fetchByApplicationPDF = CRMcontroller.fetchByApplicationPDF(string2, headerPersist,
				X_From_ID, X_To_ID, X_Transaction_ID, X_User_ID, X_Request_ID, req, res);
		logger.debug("pdf generation end");
		...
		// String message
		// =aocpvImageService.savePdf(image,document,applicationNoInLong);
		...
		findByApplicationNo.setTargetAccount(account);
