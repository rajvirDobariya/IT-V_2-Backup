			...
			if (X_Request_ID.equals("NOVOPAY")) {
				...
				if (Integer.parseInt(decryptCallId) > 3) {
					...
					String validate = "{\r\n"
							...
							+ "}";
					JSONObject sendsms = new JSONObject(validate);
					...
					if (sendsms != null) {
						...
						if (Data1.has("Data")) {
							...
							if (product.equalsIgnoreCase("TW")) {
								...
								application = twowheelerDetailsService.createApplicationNo(mobileNo, product,
										branchCode, listType, customerId, X_User_ID);
							} else if (product.equalsIgnoreCase("VL")) {
								application = applicationDetailsService.createApplicationNo(mobileNo, product,
										branchCode);
							}
