...
@Service
public class LivenessImageServiceImpl implements LivenessImageService {

	private static Logger logger = LoggerFactory.getLogger(LivenessImageServiceImpl.class);

	@Override
	public JSONObject checkLivenessOfImage(String encoded, JSONObject header) {

		JSONObject sendResponse = new JSONObject();
		JSONObject request = new JSONObject();
		request.put("fileBase64", encoded);
		...
		try {
			...
			logger.debug(x.BASEURLV2 + "karza/liveness/detection?api_key=" + x.api_key);

			obj = new URL(x.BASEURLV2 + "karza/liveness/detection?api_key=" + x.api_key);
