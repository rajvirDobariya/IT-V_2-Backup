...
//			kyc1.put("verification_id", "12342Ak");
			kyc1.put("kyc_name", pandetail.getString("NameOnCard"));
			...
			kyc2.put("kyc_type", 5);
			kyc2.put("identifier", voterResponse.getJSONObject("result").getString("name"));
...
//			obj = new URL(x.onefin + "api/v1/client/loan/"+leadId+"/approve/");
	...
	public JSONObject equifaxReport(String applicationNo, JSONObject header, String leadId) {
		...
		try {
			...
			HttpsURLConnection.setDefaultHostnameVerifier(allHostsValid);
			logger.debug(x.onefin + "api/v2/call/api/" + leadId + "/get_bureau_response");
			obj = new URL(x.onefin + "api/v2/call/api/" + leadId + "/get_bureau_response");
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
			con.setRequestMethod("POST");
			con.setRequestProperty("Content-Type", "application/json");
			// con.setRequestProperty("X-Correlation-ID",
