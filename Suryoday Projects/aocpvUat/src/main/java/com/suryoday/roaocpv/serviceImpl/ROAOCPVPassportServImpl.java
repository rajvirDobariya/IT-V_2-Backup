...
public class ROAOCPVPassportServImpl implements ROAOCPVPassportService {
	...
	public JSONObject verifyPassport(String passportNo, String doi, JSONObject header, String applicationNo) {
		...
		JSONObject request = getRequest(passportNo, doi, applicationNo);
		...
		try {
			...
			logger.debug(x.BASEURL2 + "karza/verify/passport?api_key=" + x.api_key);
			obj = new URL(x.BASEURL2 + "karza/verify/passport?api_key=" + x.api_key);
	...
	private JSONObject getRequest(String passportNo, String doi, String applicationNo) {
		JSONObject parent = new JSONObject();
		ApplicationDetails fetchByApplicationId = applicationDetailsService.fetchByApplicationId(applicationNo);
		parent.put("consent", "");
		parent.put("fileNo", "");
		parent.put("dob", fetchByApplicationId.getUpdateDatets());
		parent.put("passportNo", passportNo);
		parent.put("doi", doi);
		parent.put("name", fetchByApplicationId.getName());
		return parent;
	}
