...
import com.suryoday.roaocpv.service.ROAPCVAadharReferenceService;

@Service
public class ROAOCPVAadharReferenceServiceImpl implements ROAPCVAadharReferenceService {

	private static Logger logger = LoggerFactory.getLogger(ROAOCPVAadharReferenceServiceImpl.class);
	...
	public JSONObject createAadharReference(JSONObject jSONObject, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURLV2 + "aadhar/reference?api_key=" + x.api_key);
			obj = new URL(x.BASEURLV2 + "aadhar/reference?api_key=" + x.api_key);
			// obj =new
			// URL("https://intramashery.suryodaybank.co.in/aadhar/reference?api_key=kyqak5muymxcrjhc5q57vz9v");
	...
	public JSONObject fetchAadharByReferenceNo(String referenceNo, JSONObject header) {
		...
		try {
			...
			HttpsURLConnection.setDefaultHostnameVerifier(allHostsValid);
			logger.debug(x.BASEURLV2 + "aadhar/reference/" + referenceNo + "?api_key=" + x.api_key);
			obj = new URL(x.BASEURLV2 + "aadhar/reference/" + referenceNo + "?api_key=" + x.api_key);
