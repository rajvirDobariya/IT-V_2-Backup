...
//		 Header.put("X-Correlation-ID",X_CORRELATION_ID );
	...
	@RequestMapping(value = "/get/accountIdsByCIFNoEncy", method = RequestMethod.POST, produces = "application/json")
	public ResponseEntity<Object> getAllAccountIdsByCIFNo(@RequestBody String bm,
			@RequestHeader(name = "X-User-ID", required = true) String X_User_ID,
			@RequestHeader(name = "X-Encode-ID", required = false) String X_encode_ID,
			@RequestHeader(name = "X-Session-ID", required = true) String X_Session_ID, HttpServletRequest req)
			throws Exception {
		logger.debug("getAllAccountIdsByCIFNo start");
		...
		if (sessionId == true) {
			...
			if (customerDetails != null) {
				...
				List<String> accountIds = new ArrayList<>();
				 ...
				 responseData.put("accountIds", accountIds);
				String encryptString2 = Crypt.encrypt(responseData.toString(), X_encode_ID);
				org.json.JSONObject data2 = new org.json.JSONObject();
				data2.put("value", encryptString2);
				org.json.JSONObject data4 = new org.json.JSONObject();
				data4.put("Data", data2);
				logger.debug("response : " + data4.toString());
				return new ResponseEntity<>(data4.toString(), HttpStatus.OK);
