...
public class DsaOnBoardPancardControllerEncy {
	...
	private static Logger logger = LoggerFactory.getLogger(DsaOnBoardPancardControllerEncy.class);
			...
			if (X_Request_ID.equals("IEXCEED")) {
				...
				if (panCardValidate != null) {
					...
					if (Data1.has("Data")) {
						...
						JSONObject responseResponse = Data1.getJSONArray("Data").getJSONObject(0)
								.getJSONArray("OutputData").getJSONObject(0);
						...
						dsaOnboardMemberService.saveResponse("panCard", panCardNo, applicationNo,
								responseResponse.toString(), member);
						if (count == 1) {
