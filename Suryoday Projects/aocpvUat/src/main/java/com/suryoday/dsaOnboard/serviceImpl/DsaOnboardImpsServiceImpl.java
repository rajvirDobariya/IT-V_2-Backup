...
public class DsaOnboardImpsServiceImpl implements DsaOnboardImpsService {
	...
	public JSONObject searchIfsc(String ifscCode, JSONObject header) {
		...
		try {
			...
			obj = new URL(x.BASEURL + "transaction/ifsc/code/v2?api_key=" + x.api_key + "&IFSCCode=" + ifscCode);
			logger.debug(x.BASEURL + "transaction/ifsc/code/v2?api_key=" + x.api_key + "&IFSCCode=" + ifscCode);
	...
	public JSONObject pennyLess(String accountNo, String ifsc, JSONObject header, String accountHolderName) {
		...
		try {
			...
			HttpsURLConnection.setDefaultHostnameVerifier(allHostsValid);
			obj = new URL(x.BASEURLV2 + "payments/imps/shiny/name?api_key=" + x.api_key);
			logger.debug(x.BASEURLV2 + "payments/imps/shiny/name?api_key=" + x.api_key);
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
