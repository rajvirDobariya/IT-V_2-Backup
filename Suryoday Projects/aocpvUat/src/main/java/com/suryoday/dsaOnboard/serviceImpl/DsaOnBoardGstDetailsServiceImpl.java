...
@Service
public class DsaOnBoardGstDetailsServiceImpl implements DsaOnBoardGstDetailsService {

	private static Logger logger = LoggerFactory.getLogger(DsaOnBoardGstDetailsServiceImpl.class);

	@Override
	public JSONObject gstAuthentication(JSONObject jsonObject, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURLV2 + "karza/gst/gsp/authentication?api_key=" + x.api_key);
			obj = new URL(x.BASEURLV2 + "karza/gst/gsp/authentication?api_key=" + x.api_key);
