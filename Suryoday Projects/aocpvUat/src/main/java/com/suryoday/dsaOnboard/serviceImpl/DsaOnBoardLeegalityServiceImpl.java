...
public class DsaOnBoardLeegalityServiceImpl implements DsaOnBoardLeegalityService {
	...
	private JSONObject getRequest(String applicationNo, String base64pdf, String documentType) {
		...
		invitees1.put("Name", "Mohit Arya");
		invitees1.put("Email", "mohit.arya@suryodaybank.com");
		invitees1.put("Phone", "8898923652");
	...
	public void updateDocumentId(JSONObject data, String applicationNo, String documentType) {
		...
		List<DsaOnBoardLeegalityInfo> listDB = leegalityRepository.getByApplicationNoAndDocument(applicationNo,
				documentType);
		if (listDB.size() == 0) {
			...
			for (int n = 0; n < invitees.length(); n++) {
				...
				if (!jsonObject.get("SignUrl").equals(null)) {
					...
					DsaOnBoardLeegalityInfo info = new DsaOnBoardLeegalityInfo(applicationNo, Active, Email, ExpiryDate,
							Name, Phone, SignUrl, documentId, "false", documentType);
	...
	public List<DsaOnBoardLeegalityInfo> getByApplicationNoAndDocument(String applicationNo, String documentType) {
		List<DsaOnBoardLeegalityInfo> list = leegalityRepository.getByApplicationNoAndDocument(applicationNo,
				documentType);
		if (list.size() == 0) {
	...
	@Override
	public List<DsaOnBoardLeegalityInfo> updateLeegalityVerify(String applicationNo, JSONObject data,
			String documentType) {
		List<DsaOnBoardLeegalityInfo> listDB = leegalityRepository.getByApplicationNoAndDocument(applicationNo,
				documentType);
		if (listDB.size() == 0) {
