...
import com.suryoday.goNogo.service.GoNogoCibilReportService;

@RestController
@RequestMapping("/goNogo")
public class GoNoGoCibilReportControllerEncy {

	@Autowired
	GoNogoCibilReportService goNogoCibilReportService;

	@Autowired
	UserService userService;

	private static Logger logger = LoggerFactory.getLogger(GoNoGoCibilReportControllerEncy.class);
	...
	public ResponseEntity<Object> createAadharReference(@RequestBody String jsonRequest,
		...
		if (X_Request_ID.equals("DSA")) {
			...
			if (sessionId == true) {
				...
				JSONObject cibilReport = goNogoCibilReportService.getCibilReport(jsonObject, Header);
