...
//		}
				...
				for (int n = 0; n < document.length(); n++) {
					...
					if (fileName.equals(fileNameInRequest)) {
						try {
							...
							int versioncode = 1;
							Optional<AocpvImages> optional = imageRepository.getByApplicationNoAndName(applicationNo,
									Documenttype);
							if (optional.isPresent()) {
					...
					for (int n = 0; n < document.length(); n++) {
						...
						if (fileName.equals(fileNameInRequest)) {
							try {
								...
								int versioncode = 1;
								Optional<AocpvImages> optional = imageRepository
										.getByApplicationNoMember(applicationNoInLong, Documenttype, member);
								if (optional.isPresent()) {
									AocpvImages aocpvImage = optional.get();
									aocpvImages.setId(aocpvImage.getId());
									versioncode = aocpvImage.getVersioncode();
					...
					if (fileName.equals(fileNameInRequest)) {
						try {
							...
							if (flowStatus.equals("UD")) {
								...
								optional = imageRepository.getByApplicationNoMember(applicationNo, Documenttype,
										member);
							}
							if (optional.isPresent()) {
								AocpvImages aocpvImage = optional.get();
	...
	@Override
	public void saveAadharPhoto(Map<String, String> files, long applicationNoInLong, JSONArray document,
			String flowStatus) {
...
//					}
				...
				if (fileName.equals(fileNameInRequest)) {
					try {
						...
						int versioncode = 1;
						Optional<AocpvImages> optional = imageRepository.getByApplicationNoAndName(applicationNoInLong,
								Documenttype);
						if (optional.isPresent()) {
							AocpvImages aocpvImage = optional.get();
							aocpvImages.setId(aocpvImage.getId());
...
// 		}
	...
	public String fetchMemberImageByApplicationNo(long applicationNoInLong, String member, String documentType) {
		Optional<AocpvImages> optional = imageRepository.getByApplicationNoMember(applicationNoInLong, documentType,
				member);
		if (optional.isPresent()) {
	...
	public String validateUDImage(long applicationNoInLong) {
		...
		if (list.size() == 3) {
			...
			for (AocpvImages aocpvImages : list) {
				...
				} else if (aocpvImages.getDocumenttype().equals("houseImageInside")) {
			...
			if (utilityBillPhoto) {
				return "Please Capture utilityBillPhoto";
			} else if (houseImageInside) {
				return "Please Capture houseImageInside";
			} else {
				return "Please Capture houseImageOutside";
			}
		}
	}
