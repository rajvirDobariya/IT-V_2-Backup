...
public class SendOtpControllerWebEncy {

	Logger logger = LoggerFactory.getLogger(SendOtpControllerWebEncy.class);
	...
	@RequestMapping(value = "/sendOtp/WEBEncy", method = RequestMethod.POST, produces = "application/json")
	...
	@RequestMapping(value = "/validateOTP/WEBEncy", method = RequestMethod.POST, produces = "application/json")
		...
		if (sessionId == true) {
			...
			if (X_Request_ID.equals("NOVOPAY")) {
				...
				if (sendsms != null) {
					...
					if (Data1.has("Data")) {
						h = HttpStatus.OK;
						String message = aocpvIncomeService.updateMobileVerify(applicationNoInLong, member);
					} else if (Data1.has("Error")) {
						h = HttpStatus.BAD_REQUEST;
