...
public class AocpvLoanCreationServiceImpl implements AocpvLoanCreationService {
	...
	public AocpvLoanCreation getByApplicationNo(long applicationNoInLong, String status) {
		Optional<AocpvLoanCreation> optional = aocpvLoanCreationRepo.getByApplicationNoAndStatus(applicationNoInLong,
				status);
		if (optional.isPresent()) {
			return optional.get();
		}
	...
	@Override
	public void updateStatusbyapplicaton(long applicationNo, String IsDmsUpload) {
		aocpvLoanCreationRepo.updateStatusbyapplicaton(applicationNo, IsDmsUpload);
	}

	@Override
	public List<AocpvLoanCreation> fetchByIsdms(String IsDmsUpload) {
		List<AocpvLoanCreation> list = aocpvLoanCreationRepo.fetchByIsdms(IsDmsUpload);
		if (list.isEmpty()) {
			throw new NoSuchElementException("List of Application Number is Empty with NO Status");
		}
		return list;
	}

	@Override
	public List<AocpvLoanCreation> fetchByIsBrImage(String IsBrNetImageUpload) {
		List<AocpvLoanCreation> list = aocpvLoanCreationRepo.fetchByIsBrImage(IsBrNetImageUpload);
		if (list.isEmpty()) {
			throw new NoSuchElementException("List of Application Number is Empty with NO Status");
		}
		return list;
