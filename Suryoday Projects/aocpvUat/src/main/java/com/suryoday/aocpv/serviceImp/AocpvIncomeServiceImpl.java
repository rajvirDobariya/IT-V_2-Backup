...
//					}
			...
			if (optional.isPresent()) {
				...
				// versionCode++;
				if (member.equalsIgnoreCase("SELF")) {
					if (!incomeDetails.getFirstName().equals(aocpvIncomeDetail.getFirstName())) {
						throw new DuplicateEntryException("please select other memeber");
					} else if (!incomeDetails.getLastName().equals(aocpvIncomeDetail.getLastName())) {
						throw new DuplicateEntryException("please select other memeber");
					}
				}
				if (versionCode == 0) {
			...
			if (aocpvIncomeDetail.getPrimarySourceOfIncome().equalsIgnoreCase("YES")) {
				List<AocpvIncomeDetails> list = aocpvIncomeDetailsRepository
						.getByApp(aocpvIncomeDetail.getApplicationNo());
				if (list.size() != 0) {
					for (AocpvIncomeDetails a : list) {
						String member2 = a.getMember();
						String member3 = aocpvIncomeDetail.getMember();
						if (a.getPrimarySourceOfIncome().equalsIgnoreCase("YES")) {
...
//		}
	...
	@Override
	public String documentVerify(long applicationNoInLong, String document, String documentType,
	...
	public List<IncomeDetailWeb> fetchByMember(long applicationNoInLong) {
		...
		for (AocpvIncomeDetails incomeDetails : list) {
			...
			List<AocpvImages> listAocpvImage = imageService.getByApplicationNoAndMember(applicationNoInLong,
	...
	public String saveIncome(AocpvIncomeDetails aocpvIncomeDetails) {
		...
		Optional<AocpvIncomeDetails> optional = aocpvIncomeDetailsRepository.find(aocpvIncomeDetails.getApplicationNo(),
				aocpvIncomeDetails.getMember());
		if (optional.isPresent()) {
			AocpvIncomeDetails incomeDetails = optional.get();
...
//		}
	...
	@Override
	public void savePanResponse(String panResponse, String applicationNo, String member, String isVerify,
			JSONObject jsonRequest) {
		Optional<AocpvIncomeDetails> optional = aocpvIncomeDetailsRepository.find(Long.parseLong(applicationNo),
				member);
		if (optional.isPresent()) {
			AocpvIncomeDetails incomeDetails = optional.get();
