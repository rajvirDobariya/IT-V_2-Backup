...
@Service
public class GetLoanDetailsServiceImpl implements GetLoanDetailsService {

	private static Logger logger = LoggerFactory.getLogger(GetLoanDetailsServiceImpl.class);
	...
	public JSONObject getLoanDetails(String accountNo, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURLV2 + "fetch/loan/account/details/" + accountNo + "?api_key=" + x.api_key);
			obj = new URL(x.BASEURLV2 + "fetch/loan/account/details/" + accountNo + "?api_key=" + x.api_key);
