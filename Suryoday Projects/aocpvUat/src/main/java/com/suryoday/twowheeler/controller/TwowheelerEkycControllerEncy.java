...
public class TwowheelerEkycControllerEncy {
	@Autowired
	...
	private static Logger logger = LoggerFactory.getLogger(TwowheelerEkycControllerEncy.class);
	...
	public ResponseEntity<Object> ekycRequest(@RequestBody String parent,
		...
		if (sessionId == true) {
			...
			logger.debug("Main Ekyc Request AADHAR : " + aadhar);
			String eKycRequest = roaocpvEkycService.getJsonRequest(pidxml, Long.toString(aadhar));

			logger.debug("Main Ekyc Request1 : " + eKycRequest);
					...
					if (!jsonResponse.equals(null)) {
						...
						if (jsonResponse.has("KycRes")) {
							...
							DBResponse.getJSONObject("Response").getJSONObject("KycRes").getJSONObject("UidData")
									.remove("Pht");
							DBResponse.put("referenceNo", uid);
							familyMemberService.saveResponse("ekyc", "FINGERPRINT", applicationNo,
									DBResponse.toString(), member);
...
//							Long applicationNo1 = Long.parseLong(applicationNo);
							...
							DBResponse.getJSONObject("Response").getJSONObject("KycRes").getJSONObject("UidData")
									.remove("Pht");
							DBResponse.put("referenceNo", uid);
							familyMemberService.saveResponse("ekyc", "fACEEKYC", applicationNo, DBResponse.toString(),
									member);
				...
				if (!HsmData.equals("")) {
					...
					if (!jsonResponse.equals(null)) {
						...
						if (jsonResponse.has("KycRes")) {
							...
							DBResponse.getJSONObject("Response").getJSONObject("KycRes").getJSONObject("UidData")
									.remove("Pht");
							DBResponse.put("referenceNo", aadharNo);
							familyMemberService.saveResponse("ekyc", "Otp", applicationNo, DBResponse.toString(),
									member);
