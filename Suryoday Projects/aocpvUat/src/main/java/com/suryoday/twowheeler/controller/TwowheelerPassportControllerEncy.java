...
public class TwowheelerPassportControllerEncy {

	@Autowired
	TwowheelerPassportService passportservice;

	@Autowired
	UserService userService;

	private static Logger logger = LoggerFactory.getLogger(TwowheelerPassportControllerEncy.class);

	@PostMapping(value = "/verifyPassportEncy", produces = "application/json")
	public ResponseEntity<Object> verifyPassport(@RequestBody String bm,
		...
		if (sessionId == true) {
			...
			JSONObject verifyPassport = passportservice.verifyPassport(passportNo, doi, Header, applicationNo, member);
