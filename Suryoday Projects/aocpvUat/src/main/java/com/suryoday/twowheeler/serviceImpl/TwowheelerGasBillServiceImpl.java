...
@Service
public class TwowheelerGasBillServiceImpl implements TwowheelerGasBillService {

	private static Logger logger = LoggerFactory.getLogger(TwowheelerGasBillServiceImpl.class);

	@Override
	public JSONObject gasBillVerify(JSONObject request) {
		...
		try {
			...
			logger.debug(x.BASEURL + "gas/bill");
			obj = new URL(x.BASEURL + "gas/bill");

			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
			con.setRequestMethod("POST");
			con.setRequestProperty("Content-Type", "application/json");
			con.setRequestProperty("api_key", x.api_key);
			con.setRequestProperty("x-karza-key", "En57jgfu0m1JFS40b645");
