...
//			 DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d/MM/yyyy");
			...
			if (branchId.isEmpty() && status.isEmpty()) {
				fetchByApplicationNo = twowheelerDetailsService.fetchByDate(startdate, enddate, assignTo);
			...
			List<TwowheelerResponse> filterList = new ArrayList<>();
			if (X_To_ID.equalsIgnoreCase("SUPERVISOR")) {
				int levelcount = Character.getNumericValue(level.charAt(6));
				for (int i = 1; i <= levelcount; i++) {
					String s = "Level " + i;
					filterList.addAll(fetchByApplicationNo.stream()
							.filter(element -> element.getLevel() != null && element.getLevel().equals(s))
							.collect(Collectors.toList()));
				}

			} else {
				if (level.equals("")) {
					filterList = fetchByApplicationNo;
				} else {
					filterList = fetchByApplicationNo.stream()
							.filter(element -> element.getLevel() != null && element.getLevel().equals(level))
							.collect(Collectors.toList());
				}
			}
			...
			org.json.simple.JSONObject response = new org.json.simple.JSONObject();
			JSONArray j = new JSONArray(filterList);
	...
	public ResponseEntity<Object> getreviewData(@RequestBody String jsonRequest,
		...
		if (sessionId == true) {
			...
			JSONObject jsonObject = new JSONObject(decryptContainerString);
			String applicationNo = jsonObject.getJSONObject("Data").getString("ApplicationNo");
			org.json.simple.JSONObject fetchByApplicationNo = twowheelerDetailsService.getreviewData(applicationNo,
					"web");
...
//			String encryptString2 = AppzillonAESUtils.encryptStringtoContainer(data, key);
	...
	public ResponseEntity<Object> fetchMemberImage(@RequestBody String jsonRequest,
		...
		if (sessionId == true) {
			...
			if (documentType.equalsIgnoreCase("pancardResponse")) {
				TwoWheelerFamilyMember memberFamilyMember = familyMemberService
						.getByApplicationNoAndMember(applicationNo, "APPLICANT");
	...
	public ResponseEntity<Object> updateStatus(@RequestBody String jsonRequest,
		...
		if (sessionId == true) {
			...
			if (jsonObject.getJSONObject("Data").has("forwardTo") && status.equalsIgnoreCase("CREDIT")) {
				String forwardTo = jsonObject.getJSONObject("Data").getString("forwardTo");
				String level = twowheelerDetailesTable.getLevel();
				twowheelerUserLogService.saveUserLog(applicationNo, level, X_User_ID, status);
				twowheelerDetailesTable.setLevel(forwardTo);
			}
			if (status.equalsIgnoreCase("APPROVED")) {
				...
				TwowheelerLoanCreation twowheelerLoanCreation = twowheelerLoanCreationService
						.fetchByapplicationNo(applicationNo);
			...
			twowheelerDetailesTable.setAssignTo("ALL");
			twowheelerDetailesTable.setUpdatedTimestamp(LocalDateTime.now());
...
//		 list.add("EbBill");
	...
	public ResponseEntity<Object> fetchLoanDetails(@RequestBody String jsonRequest,
		...
		if (sessionId == true) {
			...
			if (twloanCreation.getListType().equalsIgnoreCase("NEW") && twloanCreation.getCustomerId() == null) {
				if (!twloanCreation.isCifCreation()) {
					JSONObject cifCreation = disbursementservice.cifCreation(applicationNo, Header);
					if (cifCreation != null) {
						String crmResp = cifCreation.getString("data");
						JSONObject cifJsonResp = new JSONObject(crmResp);
						if (cifJsonResp.has("Data")) {
							String cifNumber = cifJsonResp.getJSONObject("Data").getString("UCIC");
							twloanCreation.setCifCreation(true);
							twloanCreation.setCustomerId(cifNumber);
							twloanCreation.setCifCreationResponse(cifJsonResp.toString());
							twloanCreation.setCifCreationRequest(cifCreation.getJSONObject("request").toString());
							twowheelerLoanCreationService.save(twloanCreation);
						} else if (cifJsonResp.has("Error")) {
							h = HttpStatus.BAD_REQUEST;
							twloanCreation.setErrorResponse(cifJsonResp.toString());
							twloanCreation.setCifCreationResponse(cifJsonResp.toString());
							twloanCreation.setCifCreationRequest(cifCreation.getJSONObject("request").toString());
							twowheelerLoanCreationService.save(twloanCreation);
				...
				if (!twloanCreation.isCRM()) {
					...
					if (crmModification != null) {
						...
						if (crmJsonResp.has("Data")) {
							...
							twloanCreation
									.setCrmModificationRequest(crmModification.getJSONObject("request").toString());
							twowheelerLoanCreationService.save(twloanCreation);
			...
			if (!twloanCreation.isCollateralDetails()) {
				...
				if (collateralDetails != null) {
					...
					if (collateralDetailsJsonResp.has("Data")) {
						h = HttpStatus.OK;
						String collateralNo = collateralDetailsJsonResp.getJSONObject("Data")
								.getString("CollateralNumber");
						twloanCreation.setCollateralNo(collateralNo);
						twloanCreation.setCollateralDetails(true);
						twloanCreation
								.setCollateralDetailsRequest(collateralDetails.getJSONObject("request").toString());
...
//						return new ResponseEntity<Object>(data3, h);
	...
	@RequestMapping(value = "/assignToEncy", method = RequestMethod.POST)
	public ResponseEntity<Object> assignTo(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
		...
		if (sessionId == true) {
			...
			String decryptContainerString = Crypt.decrypt(encryptString, X_encode_ID);
			JSONObject jsonObject = new JSONObject(decryptContainerString);
			String jsonArray = jsonObject.getJSONObject("Data").getJSONArray("applicationNo").toString();
			String assignTo = jsonObject.getJSONObject("Data").getString("assignTo");
			ObjectMapper objectMapper = new ObjectMapper();
			try {
				List<String> list = objectMapper.readValue(jsonArray, new TypeReference<List<String>>() {
				});
				twowheelerDetailsService.updateAssignTo(list, assignTo);
			} catch (Exception e) {
				e.printStackTrace();
			}
			org.json.simple.JSONObject response = new org.json.simple.JSONObject();
			...
			response.put("status", HttpStatus.OK.toString());
			response.put("message", "update succesfully");
			logger.debug(response.toString());
			logger.debug("PD end");
			String data = response.toString();
			String encryptString2 = Crypt.encrypt(data, X_encode_ID);
	...
	public ResponseEntity<Object> saveBankDetails(@RequestBody String jsonRequest,
		...
		if (sessionId == true) {
			...
			JSONObject jsonObject = new JSONObject(decryptContainerString);
			String applicationNo = jsonObject.getJSONObject("Data").getString("applicationNo");
			JSONArray bankDetails = jsonObject.getJSONObject("Data").getJSONArray("bankDetails");

			TwowheelerDetailesTable twowheelerDetailesTable = twowheelerDetailsService.getByApplication(applicationNo);
			twowheelerDetailesTable.setBankDetails(bankDetails.toString());
			String saveData = twowheelerDetailsService.saveData(twowheelerDetailesTable);
	...
	@RequestMapping(value = "/getBreResponseEncy", method = RequestMethod.POST)
	public ResponseEntity<Object> getBreResponse(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
		...
		if (sessionId == true) {
			...
			TwowheelerDetailesTable twowheelerDetails = twowheelerDetailsService.getByApplication(applicationNo);
			String breResponse = twowheelerDetails.getBreResponse();
	...
	@RequestMapping(value = "/getDealSummaryEncy", method = RequestMethod.POST)
	public ResponseEntity<Object> getDealSummary(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
		...
		if (sessionId == true) {
			...
			org.json.simple.JSONObject dealSummary = twowheelerDetailsService.getDealSummary(applicationNo);
			org.json.simple.JSONObject response = new org.json.simple.JSONObject();
			JSONObject j = new JSONObject(dealSummary);
			response.put("status", HttpStatus.OK.toString());
			response.put("Data", j);
			String data = response.toString();
			String encryptString2 = Crypt.encrypt(data, X_encode_ID);
	...
	@RequestMapping(value = "/addSanctionConditionEncy", method = RequestMethod.POST)
	public ResponseEntity<Object> addSanctionCondition(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
		...
		if (sessionId == true) {
			...
			JSONObject jsonObject = new JSONObject(decryptContainerString);
			String applicationNo = jsonObject.getJSONObject("Data").getString("applicationNo");
			JSONArray jsonArray = jsonObject.getJSONObject("Data").getJSONArray("sanctionCondition");
			TwowheelerDetailesTable twowheelerDetails = twowheelerDetailsService.getByApplication(applicationNo);
			twowheelerDetails.setSanctionCondition(jsonArray.toString());
	...
	@RequestMapping(value = "/addManualDeviationEncy", method = RequestMethod.POST)
	public ResponseEntity<Object> addManualDeviation(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
		...
		if (sessionId == true) {
			...
			JSONObject jsonObject = new JSONObject(decryptContainerString);
			String applicationNo = jsonObject.getJSONObject("Data").getString("applicationNo");
			JSONArray jsonArray = jsonObject.getJSONObject("Data").getJSONArray("manualDeviation");
			TwowheelerDetailesTable twowheelerDetails = twowheelerDetailsService.getByApplication(applicationNo);
			twowheelerDetails.setManualDeviation(jsonArray.toString());
			twowheelerDetailsService.saveData(twowheelerDetails);
			org.json.simple.JSONObject response = new org.json.simple.JSONObject();
	...
	@RequestMapping(value = "/getUserlogsEncy", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/changeLoanAmountEncy", method = RequestMethod.POST)
	public ResponseEntity<Object> changeLoanAmount(@RequestBody String jsonRequest,
			@RequestHeader(name = "X-Correlation-ID", required = true) String headerPersist,
			@RequestHeader(name = "X-From-ID", required = true) String X_From_ID,
			@RequestHeader(name = "X-To-ID", required = true) String X_To_ID,
			@RequestHeader(name = "X-Transaction-ID", required = true) String X_Transaction_ID,
	...
	@RequestMapping(value = "/saveReferenceInfoEncy", method = RequestMethod.POST)
