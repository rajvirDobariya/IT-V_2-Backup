...
import com.suryoday.twowheeler.repository.AssetToSchemeMasterRepo;
import com.suryoday.twowheeler.repository.DealerMasterRepo;
import com.suryoday.twowheeler.repository.LtvPolicyMasterRepo;
import com.suryoday.twowheeler.repository.NtbSchemeMasterRepo;
import com.suryoday.twowheeler.repository.SchemeMasterRepo;
import com.suryoday.twowheeler.repository.SchemeToLtvMasterRepo;
...
public class TwoWheelerMastersServImpl implements TwoWheelerMastersService {
	...
	@Autowired
	private TwoWheelerMastersRepo twowheelermastersrepo;

	@Autowired
	private SchemeMasterRepo schememastersrepo;

	@Autowired
	private DealerMasterRepo dealermasterrepo;

	@Autowired
	private TWAssetMasterRepository assetRepo;

	@Autowired
	private AssetToSchemeMasterRepo assetToSchemeMasterRepo;

	@Autowired
	private NtbSchemeMasterRepo ntbSchemeMasterRepo;

	@Autowired
	private LtvPolicyMasterRepo ltvPolicyMasterRepo;

	@Autowired
	private SchemeToLtvMasterRepo schemeToLtvMasterRepo;
...
//	}
	...
	public List<DealerByPincode> fetchAllDealer(String branchId, String manufacturer) {
		...
		if (list.isEmpty()) {
			throw new NoSuchElementException("NO Dealer Found");
		}
		return list;
	}

	@Override
	public List<String> getSchemeNtb(String model, String breCustomerCategory) {
		if (model != null && breCustomerCategory != null) {
			Optional<List<String>> optional = assetToSchemeMasterRepo.getByModelAndCreditStatusDescription(model,
					breCustomerCategory);
			return optional.orElseThrow(() -> new NoSuchElementException("NO Dealer Found"));
		} else {
			throw new EmptyInputException("Please select asset first");
		}
	}

	@Override
	public NtbSchemeMaster fetchBySchemeCodeNtb(String schemeName) {
		Optional<NtbSchemeMaster> optional = ntbSchemeMasterRepo.getBySchemeDescription(schemeName);
		return optional.orElseThrow(() -> new NoSuchElementException("NO Scheme Found"));
	}

	@Override
	public String getLtv(String scheme, String breCustCatagory) {
		Optional<String> optional = ltvPolicyMasterRepo.getByLtvPolicyDescription(scheme);
		if (optional.isPresent()) {
			Optional<String> optional1 = schemeToLtvMasterRepo.getLtv(optional.get(), breCustCatagory);
