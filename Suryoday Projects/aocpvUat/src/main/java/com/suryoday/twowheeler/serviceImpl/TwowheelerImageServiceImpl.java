...
public class TwowheelerImageServiceImpl implements TwowheelerImageService {
	...
	@Override
	public void savePhoto(MultipartFile[] files, String applicationNo, JSONArray document, String flowStatus,
			String member) {
...
//					twowheelerImage.setVersioncode(versioncode);
			...
			for (int n = 0; n < document.length(); n++) {
				...
				if (fileName.equals(fileNameInRequest)) {
					try {
						...
						Optional<TwowheelerImage> optional = imageRepository
								.getByApplicationNoAnddocumentType(applicationNo, Documenttype);
						if (optional.isPresent()) {
							TwowheelerImage aocpvImage = optional.get();
	...
	public void savepdf(byte[] image, JSONObject object, String applicationNo) {
		...
		try {
			...
			int versioncode = 1;
			Optional<TwowheelerImage> optional = imageRepository.getByApplicationNoAnddocumentType(applicationNo,
					Documenttype);
			if (optional.isPresent()) {
				TwowheelerImage aocpvImage = optional.get();
				TWImages.setId(aocpvImage.getId());
			...
			if (!document.isEmpty()) {
				...
				if (fileName.equals(fileNameInRequest)) {
					try {
						...
						if (Documenttype.equalsIgnoreCase("customerPhoto")) {
							Optional<TwowheelerImage> optional = imageRepository
									.getByApplicationNoAnddocumentType(applicationNo, Documenttype);
							if (optional.isPresent()) {
								TwowheelerImage aocpvImage = optional.get();
...
//		}
	...
	@Override
	public void deleteImage(String applicationNo, String imageName) {
		imageRepository.deleteImage(applicationNo, imageName);
