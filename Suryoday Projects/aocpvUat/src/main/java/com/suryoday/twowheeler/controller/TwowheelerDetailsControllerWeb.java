...
//			 DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d/MM/yyyy");
		...
		if (branchId.isEmpty() && status.isEmpty()) {
			...
			fetchByApplicationNo = twowheelerDetailsService.fetchByDateWithStatus(startdate, enddate, status, assignTo);
		} else if (status.isEmpty()) {
			fetchByApplicationNo = twowheelerDetailsService.fetchByDateWithBranchId(startdate, enddate, branchId,
					assignTo);
		} else {
			fetchByApplicationNo = twowheelerDetailsService.fetchByDate(startdate, enddate, status, branchId, assignTo);
		}
	...
	public ResponseEntity<Object> fetchMemberImage(@RequestBody String jsonRequest,
		...
		if (documentType.equalsIgnoreCase("pancardResponse")) {
			TwoWheelerFamilyMember memberFamilyMember = familyMemberService.getByApplicationNoAndMember(applicationNo,
					"APPLICANT");
	...
	public ResponseEntity<Object> updateStatus(@RequestBody String jsonRequest,
		...
		if (status.equalsIgnoreCase("APPROVED")) {
			...
			twowheelerSendSMS.sendSms(twowheelerDetailesTable, status);
		}
...
//		 list.add("EbBill");
	...
	public ResponseEntity<Object> fetchLoanDetails(@RequestBody String jsonRequest,
			...
			if (!twloanCreation.isCifCreation()) {
				...
				if (cifCreation != null) {
					...
					if (cifJsonResp.has("Data")) {
						...
						twloanCreation.setCifCreationRequest(cifCreation.getJSONObject("request").toString());
						twowheelerLoanCreationService.save(twloanCreation);
						return new ResponseEntity<Object>(cifJsonResp.toString(), h);
					}
				} else {
		...
		if (!twloanCreation.getSpdc()) {
			JSONObject spdc = disbursementservice.spdc(twloanCreation, Header);
			if (spdc != null) {
				...
				JSONObject Data1 = new JSONObject(Data2);
				if (Data1.has("Data")) {
					String referenceNo = Data1.getJSONObject("Data").getString("ReferenceNumber");

					twloanCreation.setSpdcReferenceNumber(referenceNo);
					twloanCreation.setSpdc(true);
					twloanCreation.setSpdcResponse(Data1.toString());
					twowheelerLoanCreationService.save(twloanCreation);
					h = HttpStatus.OK;
				} else if (Data1.has("Error")) {
					h = HttpStatus.BAD_REQUEST;
	...
	@RequestMapping(value = "/getBreResponse", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/getDealSummary", method = RequestMethod.POST)
	public ResponseEntity<Object> getDealSummary(@RequestBody String jsonRequest,
		...
		org.json.simple.JSONObject response = new org.json.simple.JSONObject();
		response.put("Data", dealSummary);
		return new ResponseEntity<Object>(response, HttpStatus.OK);
	}

	@RequestMapping(value = "/addSanctionCondition", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/addManualDeviation", method = RequestMethod.POST)
