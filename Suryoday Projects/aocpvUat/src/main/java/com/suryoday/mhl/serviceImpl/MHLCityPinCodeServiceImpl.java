...
import com.suryoday.mhl.service.MHLCityPinCodeService;

@Service
public class MHLCityPinCodeServiceImpl implements MHLCityPinCodeService {

	private static Logger logger = LoggerFactory.getLogger(MHLCityPinCodeServiceImpl.class);
	...
	public JSONObject getCityPincode(String cityPincode, JSONObject header) {
		...
		try {
			...
			logger.debug(x.BASEURLV2 + "custRef/PinCode/" + cityPincode + "/search?api_key=" + x.api_key);
			obj = new URL(x.BASEURLV2 + "custRef/PinCode/" + cityPincode + "/search?api_key=" + x.api_key);
