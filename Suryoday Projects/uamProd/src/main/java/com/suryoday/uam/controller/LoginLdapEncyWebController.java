...
//			
			...
			if (serviceStatus.equals("00")) {
				...
				if (fetchById == null) {
					...
					newUser.setUserAccess("[LEAD-MANAGEMENT,INSURANCE,COMMON]");
...
//					List<UserAccessResponse> collect = map.stream().filter(entity->module.contains(entity.getModuleName())).collect(Collectors.toList());
				List<UserAccessResponse> filterMap = new ArrayList<>();
				JSONArray jsonArray = new JSONArray(fetchById.getUserAccess());
				for (UserAccessResponse keyMap : map) {
					for (int i = 0; i < jsonArray.length(); i++) {
						String element = jsonArray.getString(i);
						if (element.equals(keyMap.getModuleName())) {
							filterMap.add(keyMap);
						}
					}
				}
				...
				JSONArray j = new JSONArray(filterMap);
