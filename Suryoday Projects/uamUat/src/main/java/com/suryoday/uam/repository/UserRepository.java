...
public interface UserRepository extends JpaRepository<User, String> {
	...
	@Query(value = "SELECT [SESSION_ID] FROM SPRING_SESSION  where PRIMARY_ID=:primaryID", nativeQuery = true)
	String getSameSessionIdByUser(String primaryID);

	@Query(value = "select [ATTRIBUTE_NAME]  from [SPRING_SESSION_ATTRIBUTES] where [SESSION_PRIMARY_ID] =:primeryId", nativeQuery = true)
	Optional<String> getUserId(String primeryId);

	@Query(value = "select [PRIMARY_ID] from SPRING_SESSION where SESSION_ID =:sessionIdReq", nativeQuery = true)
	Optional<String> getPrimeryId(String sessionIdReq);

	@Query(value = "SELECT * FROM [suryodayaopcv].[dbo].[tbl_user_permission]", nativeQuery = true)
	public List<Object[]> findAllUserPermission();
}
