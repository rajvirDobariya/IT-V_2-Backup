...
public class EncryptDecryptHelper {
	...
	@Autowired
	private EndUseActivityRepo endUseActivityRepo;
	...
	public  JSONObject validateHeadersAndSessionId(String encryptRequestString, String channel,
			String X_Session_ID) {
		...
		if (encryptRequestString == null || encryptRequestString.isEmpty() || channel == null || channel.isEmpty()
			...
			LOG.debug("Validate Headers :" + message);
			throw new EndUseException("Validate Headers :" + message);
		}
		...
		if (dbSessionId == null || dbSessionId.isEmpty()) {
			...
			throw new EndUseException("SessionId is expired or Invalid sessionId!");
		}
	...
	public String fetchSessionId(String sessionId) {
		...
		String fetchSessionId = endUseActivityRepo.getAllSessionIds(sessionId);
		System.out.println("SESSION ID 2:" + fetchSessionId);
